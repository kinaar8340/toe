...
def small_rotor(angle_rad, axis):
    ...
    s = torch.sin(half)
    return torch.tensor(
        [c, axis[0] * s, axis[1] * s, axis[2] * s], dtype=torch.float32, device=axis.device
    )


def safe_cosine(a: torch.Tensor, b: torch.Tensor) -> torch.Tensor:
    """Safe cosine similarity that works on any device/shape."""
...
class CubeChain:
    ...
    def __init__(self, num_cubes: int = 12, device=None):
        ...
        self.embeddings: list[torch.Tensor | None] = [None] * num_cubes
        self.orientations = torch.zeros(num_cubes, dtype=torch.long, device=self.device)
        self.parents = [-1] * num_cubes
        self.dual_vectors: list[torch.Tensor | None] = [None] * num_cubes
...
class RingConeChain(nn.Module):
    RING_SIZES: ClassVar[list[int]] = [24, 21, 18, 15, 12, 9, 6, 3]
    ...
    def __init__(self, embed_dim: int = 384, device=None):
        ...
        self.embed_dim = embed_dim
        self.rings = [
            CubeChain(num_cubes=size, device=self.device)
            for size in self.RING_SIZES + self.RING_SIZES
        ]
        self.register_buffer(
            "ring_polarities", torch.zeros(self.TOTAL_CUBES, dtype=torch.long, device=self.device)
        )
        self.register_buffer(
            "face_grids",
    ...
    def get_stats(self):
        ...
        for ring in self.rings:
            for emb, dual in zip(ring.embeddings, ring.dual_vectors, strict=False):
...
class TwistedHelicalConduit(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.register_buffer(
            "vortex_phase", torch.zeros(self.num_pol, dtype=torch.long, device=self.device)
        )
    ...
    # === ALL VORTEX / FIB / GOLDEN / 369 HELPERS ===
    ...
    def golden_scale(self, base: float, fib_index: int = 8) -> float:
        """Scale value using Fibonacci ratio (approximates golden ratio)."""
        f_n = self.fib(fib_index)
        f_np1 = self.fib(fib_index + 1)
        fib_ratio = f_np1 / f_n if f_n != 0 else getattr(self, "PHI", (1 + 5**0.5) / 2)
        return base * fib_ratio

    def get_vortex_digit_fib(self, pol_idx: int = 0, s: float = 0.0, fib_index: int = 13) -> int:
        """Return vortex math digit (1-9) using golden fib scaling + polarization offset.
        Robust fallback that works even if vortex_phase/offset are missing."""
    ...
    def _toroidal_wrap(self, s: float) -> float:
        """Toroidal modulo-9 wrap for closed periodic boundary S¹ X S¹."""
        if not self.toroidal_modulo9:
            return s
        period = self.max_depth * 9.0
        return s % period

    def _clifford_4d_coords(self, s: float, pol_idx: int = 0) -> torch.Tensor | None:
        """4D product manifold S¹ X S¹ (zero Gaussian curvature — UIUC MA198-2012)."""
        ...
        v_digit = torch.tensor(
            self.get_vortex_digit_fib(pol_idx, s), dtype=torch.float32, device=self.device
        )
    ...
    def position(self, s: float, pol_idx: int = 0) -> torch.Tensor:
        s_t = torch.as_tensor(
            self._toroidal_wrap(s), dtype=torch.float32, device=self.device
        ).clamp_(0.0, self.max_depth)
        ...
        q_rot = qnormalize(
            torch.tensor(
                [torch.cos(q_angle), 0.0, 0.0, torch.sin(q_angle)],
                dtype=torch.float32,
                device=self.device,
            )
        )
        ...
        quat_residual = self.quat_spine(
            torch.zeros(self.quat_logical_dim, device=device) * self.quat_scale.to(device)
        )
    ...
    def read(
        ...
        s_query = torch.tensor(
            self._toroidal_wrap(s_query), dtype=torch.float32, device=self.device
        ).clamp_(0.0, self.max_depth)
        ss = torch.linspace(
            s_query - bandwidth, s_query + bandwidth, num_samples, device=self.device
        )
    ...
    def training_step(self, inputs: list[dict], optimizer, **kwargs) -> dict[str, float]:
        ...
        for item in inputs:
            ...
            item_loss = (
                kwargs.get("recon_weight", 4200.0) * recon
        ...
        winding_loss = braiding_loss = depth_loss = torch.tensor(
        ...
        if kwargs.get("winding_weight", 48.0) > 0:
            ...
            winding_loss = ((effective / geometric) - 1.0).pow(2) * kwargs.get(
                "winding_weight", 48.0
            )
            winding_loss = (
                winding_loss + 0.5 * torch.abs(effective - geometric) * 24.0
            )  # extra linear pull
        ...
        if kwargs.get("depth_pull_weight", 9200.0) > 0:
            ...
            for item in inputs:
                ...
                s_target = torch.tensor(
                    self._toroidal_wrap(item["s"]), dtype=torch.float32, device=self.device
                )
                pol = item.get("pol_idx", 0)
    ...
    def bake_to_cube(self, cube_idx: int, emb: torch.Tensor, orientation: int | None = None):
    ...
    def monitor_topological_winding(self, n_samples: int = 512, pol_ref: int = 0):
        ...
        def safe_float(val):
            if isinstance(val, (int | float)):  # NEW was (int, float)
            ...
            stats.update(
                {
                    "toroidal_winding": float(
                        self.max_depth * self.twist_rate * 9.0 / (2 * math.pi)
                    ),
                }
            )
    ...
    def get_helix_3d(self, s: float, pol_idx: int = 0) -> torch.Tensor:
        ...
        s_t = torch.as_tensor(s, dtype=torch.float32, device=self.device).clamp_(
            0.0, self.max_depth
        )
        s_norm = s_t / self.max_depth
        s_float = float(s)
...
class RubikConeConduit(TwistedHelicalConduit):
    ...
    def forward(
        self, face_grids: torch.Tensor, orientations: torch.Tensor, vortex_digits: torch.Tensor
    ) -> torch.Tensor:
    ...
    def epoch_synchronous_bake(self, idx: int, emb: torch.Tensor):
        ...
        rotor = small_rotor(
            phase * self.gauge_strength, torch.tensor([1, 0, 0], device=self.device)
        )
        q = q_mult(q, rotor)
        q = q_normalize(q)
...
class VQCEnhancedHelicalConduit(TwistedHelicalConduit):
    ...
    def position(self, s: float, pol_idx: int = 0) -> torch.Tensor:
        ...
        oam_phase = torch.tensor(
            s * self.oam_freq.item() + pol_idx * 3.0, device=self.device, dtype=torch.float32
        )
...
class MinimalCopresheafTNN(nn.Module):
    ...
    def forward(
        ...
        ring_polarities: torch.Tensor | None = None,
    ) -> torch.Tensor:
...
class CopresheafDiffusionStack(nn.Module):
    ...
    def __init__(
        ...
        self.res_proj = (
            nn.Linear(in_channels, out_channels) if in_channels != out_channels else None
        )
        self.to(self.device)
...
if __name__ == "__main__":
    conduit = RubikConeConduit(
        toroidal_modulo9=True, vortex_math_369=False, clifford_projection=True
    )
